...
import { View, Text, StyleSheet, FlatList, Modal, TextInput, Button,
...
export default function GamesScreen() {
  ...
  useEffect(() => {
    const fetchGames = async () => {
      try {
        ...
        const gamesList = gamesSnapshot.docs.map(doc => ({ id: doc.id,
          ...doc.data() }));
        setGames(gamesList);
      } catch (error) {
  ...
  const handleAddGame = async () => {
    if (gameTitle && gameDescription && gameImageUrl) {
      const newGame = { title: gameTitle, description: gameDescription,
      ...
      try {
        ...
        setGameDescription('');
        setGameImageUrl('');
        Alert.alert("Igra dodana", "Vaša igra je uspješno dodana!");
      } catch (error) {
        console.error("Error adding game: ", error);
        Alert.alert("Greška", "Došlo je do greške pri dodavanju igre.");
      }
    } else {
      alert('Molimo unesite sve podatke o igrici.');
    }
  };
      ...
      <Modal visible={modalVisible} animationType="slide" transparent={true}>
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            ...
            <TextInput placeholder="Naslov" style={styles.input}
              value={gameTitle} onChangeText={setGameTitle} />
            <TextInput placeholder="Opis" style={styles.input}
              value={gameDescription} onChangeText={setGameDescription} multiline />
            <TextInput placeholder="URL slike" style={styles.input}
              value={gameImageUrl} onChangeText={setGameImageUrl} />
...
const styles = StyleSheet.create({
  ...
  addButton: { backgroundColor: '#2196F3', padding: 10, borderRadius: 5,
    alignItems: 'center', marginBottom: 20 },
  addButtonText: { color: 'white', fontSize: 16 },
  gameItem: { flexDirection: 'row', alignItems: 'center', marginBottom:
    15, padding: 10, backgroundColor: '#f0f0f0', borderRadius: 5 },
  ...
  modalContainer: { flex: 1, justifyContent: 'center', alignItems:
    'center', backgroundColor: 'rgba(0,0,0,0.5)' },
  modalContent: { width: '80%', padding: 20, backgroundColor: 'white',
    borderRadius: 10 },
